...
export function aiNotConfiguredMessage() {
  return "AI verification is not configured on the server. Set OPENAI_API_KEY in the KYC backend environment.";
}
...
export async function openAiJson({ system, user, images = [], maxTokens = 600, temperature = 0.1 }) {
  ...
  if (!res.ok) {
    ...
    const quota = /insufficient_quota|credit_balance|no credits/i.test(detail);
    const err = new Error(
      res.status === 401
        ? "AI service credentials are invalid."
        : res.status === 429
        ? quota
          ? "AI verification is temporarily unavailable. Please try again later."
          : "AI service is busy right now. Please try again shortly."
        : `AI request failed${detail ? `: ${detail}` : ` (${res.status})`}`
    );
...
export async function verifySubmissionWithAi({ documentUrl, livenessUrls, stepActions = [] }) {
  ...
  for (let i = 0; i < lives.length; i++) {
    ...
    try {
      const evaluated = await analyzeLivenessFrames(action, [lives[i]]);
